# $Id: controller.pm, v0.0.2 06/23/2007 12:05:00 dan stephenson ( ispyhumanfly ) Exp$
#
...
sub OH_fixup {
    ...
    $self->logger->debug('nuport :: debug level error handling initialized');
    if ( $self->page_error ) { $self->logger->error( 'nuport :: there is a problem! - ' . "$!" ) }
...
sub OH_cleanup {
    ...
    # here is the magic of nuport.  the following methods will
    ...
    $self->{model}->do_rss();    # update nuport RSS subscription file...
